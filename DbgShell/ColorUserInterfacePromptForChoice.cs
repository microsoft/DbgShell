...
{
    internal partial class ColorHostUserInterface : PSHostUserInterface, IHostUISupportsMultipleChoiceSelection
    {
        /// <summary>
        /// 
        /// See base class
        ...
        {
            Util.Assert(defaultChoiceKeys != null, "defaultChoiceKeys cannot be null.");
            ...
            if (defaultChoiceKeys.Count > 0)
            {
                ...
                if (defaultChoiceKeys.Count == 1)
                {
                    defaultPrompt = shouldEmulateForMultipleChoiceSelection ?
                        Util.Sprintf(ConsoleHostUserInterfaceStrings.DefaultChoiceForMultipleChoices, defaultChoices)
                        :
                        Util.Sprintf(ConsoleHostUserInterfaceStrings.DefaultChoicePrompt, defaultChoices);
                }
                else
                {
                    defaultPrompt = Util.Sprintf(ConsoleHostUserInterfaceStrings.DefaultChoicesForMultipleChoices,
                        defaultChoices);
                }
            }
        ...
        private const string EmptyChoicesErrorTemplateResource = "\"{0}\" should have at least one element.";
        private const string InvalidDefaultChoiceErrorTemplateResource = "\"{0}\" must be a valid index into \"{1}\" or -1 for no default choice.";
    }
}   // namespace 
