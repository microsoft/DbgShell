...
namespace MS.DbgShell
{
    ...
    internal class PSVersionInfo
    {
        ...
        internal static string GetCommitInfo()
        {
            try
            {
                string assemblyPath = System.IO.Path.GetDirectoryName(typeof(PSVersionInfo).GetTypeInfo().Assembly.Location);
                return (System.IO.File.ReadAllLines(System.IO.Path.Combine(assemblyPath, "powershell.version"))[0]);
            }
            catch (Exception e)
            {
                return e.Message;
    ...
    public sealed class SemanticVersion : IComparable, IComparable<SemanticVersion>, IEquatable<SemanticVersion>
    {
        ...
        {
            if (major < 0) throw new ArgumentException(nameof(major));
            if (minor < 0) throw new ArgumentException(nameof(minor));
            if (patch < 0) throw new ArgumentException(nameof(patch));
        ...
        {
            if (version.Revision > 0 || version.Build < 0) throw new ArgumentException(nameof(version));
        ...
        public override string ToString()
        {
            if (Patch < 0)
            {
                return string.IsNullOrEmpty(Label)
                    ? Util.Sprintf("{0}.{1}", Major, Minor)
                    : Util.Sprintf("{0}.{1}-{2}", Major, Minor, Label);
            }

            return string.IsNullOrEmpty(Label)
                ? Util.Sprintf("{0}.{1}.{2}", Major, Minor, Patch)
                : Util.Sprintf("{0}.{1}.{2}-{3}", Major, Minor, Patch, Label);
        }
        ...
        public static bool operator <(SemanticVersion v1, SemanticVersion v2)
        {
            if ((object)v1 == null) throw new ArgumentException(nameof(v1));
            ...
            internal Exception GetVersionParseException()
            {
                switch (_failure)
                {
                    ...
                    case ParseFailureKind.ArgumentOutOfRangeException:
                        throw new ValidationMetadataException( "Out of range: " + _exceptionArgument );
