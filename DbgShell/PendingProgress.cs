...
namespace MS.DbgShell
{
    ...
    class PendingProgress
    {
...
#if DEBUG || ASSERTIONS_TRACE
            Util.Assert(_nodeCount == this.CountNodes(), "We've lost track of the number of nodes in the tree");
            Util.Assert(_nodeCount <= maxNodeCount, "Too many nodes in tree!");
#endif
        }
        ...
        FindOldestLeafmostNodeHelper(ArrayList treeToSearch, out ArrayList listWhereFound, out int indexWhereFound)
        {
            ...
            if (v.FoundNode == null)
            {
                Util.Assert(listWhereFound == null, "list should be null when no node found");
                Util.Assert(indexWhereFound == -1, "index should indicate no node found");
                Util.Assert(_topLevelNodes.Count == 0, "if there is no oldest node, then the tree must be empty");
                Util.Assert(_nodeCount == 0, "if there is no oldest node, then the tree must be empty");
            }
#endif
            return v.FoundNode;
        }
        ...
        {
            Util.Assert(_topLevelNodes != null, "Shouldn't need to render progress if no data exists");
            Util.Assert(maxWidth > 0, "maxWidth is too small");
            Util.Assert(maxHeight >= 3, "maxHeight is too small");
        ...
        {
            Util.Assert(strings != null, "strings should not be null");
            Util.Assert(nodes != null, "nodes should not be null");
        ...
        {
            Util.Assert(_topLevelNodes != null, "Shouldn't need to compress if no data exists");
            ...
            Util.Assert(false, "with all nodes invisible, we should never reach this point.");
    ...
    static class ProgressNodeStrings
    {
        public static string InvisibleNodesMessageSingular = "{0} activity not shown...";
        public static string InvisibleNodesMessagePlural = "{0} activities not shown...";
        public static string SecondsRemaining = "{0}{1} remaining. ";
    }
}   // namespace 
