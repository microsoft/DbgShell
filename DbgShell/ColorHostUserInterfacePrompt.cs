...
namespace MS.DbgShell
{
    internal partial class ColorHostUserInterface : PSHostUserInterface
    {
        ...
        Prompt(string caption, string message, Collection<FieldDescription> descriptions)
        {
            ...
            {
                throw new ArgumentException( Util.Sprintf( PromptEmptyDescriptionsErrorTemplateResource, "descriptions" ), "descriptions" );
            }
            ...
            lock (_instanceLock)
            {
                ...
                foreach (FieldDescription desc in descriptions)
                {
                    ...
                    if (desc == null)
                    {
                        throw new ArgumentException( Util.Sprintf( NullErrorTemplateResource, string.Format( CultureInfo.InvariantCulture, "descriptions[{0}]", descIndex ) ), "descriptions" );
                    }
                    ...
                    if (string.IsNullOrEmpty(desc.ParameterAssemblyFullName))
                    {
                        ...
                        throw new ArgumentException( Util.Sprintf( NullOrEmptyErrorTemplateResource, paramName ), paramName );
                    }
                    ...
                    if(fieldType == null)
                    {
                        ...
                        {
                            string errMsg =
                                Util.Sprintf(ConsoleHostUserInterfaceStrings.PromptTypeLoadErrorTemplate,
                    ...
                    if (fieldType.GetInterface(typeof(IList).FullName) != null)
                    {
                        ...
                        if (fieldType.IsArray)
                        {
                            ...
                            if (rank <= 0)
                            {
                                string msg = Util.Sprintf(ConsoleHostUserInterfaceStrings.RankZeroArrayErrorTemplate, desc.Name);
                                ArgumentException innerException = new ArgumentException( "Invalid argument.",
                    ...
                    {
                        string printFieldPrompt = Util.Sprintf(ConsoleHostUserInterfaceStrings.PromptFieldPromptInputSeparatorTemplate,
                            fieldPrompt);
                        // field is not a list
                        Object convertedObj = null;
                        Boolean dummy = false;
                    ...
                    if (cancelInput)
                    {
                        ...
                        LogManager.Trace( "Prompt canceled." );
                        WriteLineToConsole();
                        results.Clear();
                        break;
                    }
        ...
        {
            Util.Assert(fieldPrompt != null, "fieldPrompt should never be null when PromptReadInput is called");
            Util.Assert(desc != null, "desc should never be null when PromptReadInput is called");
            ...
            while (!inputDone)
            {
                ...
                else
                {
                    ...
                    Util.Assert(userInputString != null, "ReadLineSafe did not return a string");
                ...
                else if (rawInputString.StartsWith(PromptCommandPrefix, false, CultureInfo.CurrentCulture))
                {
        ...
        {
            Util.Assert(fieldType != null, "fieldType should never be null when PromptTryConvertTo is called");
        ...
        {
            Util.Assert(input != null && input.StartsWith(PromptCommandPrefix,StringComparison.OrdinalIgnoreCase),
                string.Format(CultureInfo.InvariantCulture, "input should start with {0}", PromptCommandPrefix));
            Util.Assert(desc != null, "desc should never be null when PromptCommandMode is called");
            ...
            if (command.Length == 1)
            {
                if (command[0] == '?')
                {
                    ...
                    {
                        string noHelpErrMsg =
                            Util.Sprintf(ConsoleHostUserInterfaceStrings.PromptNoHelpAvailableErrorTemplate, desc.Name);
        ...
        {
            string msg = Util.Sprintf(ConsoleHostUserInterfaceStrings.PromptUnrecognizedCommandErrorTemplate, command);
        ...
        private const string PromptEmptyDescriptionsErrorTemplateResource = "The \"{0}\" collection must have at least one element."; //"PromptEmptyDescriptionsErrorTemplate";
        private const string NullOrEmptyErrorTemplateResource = "\"{0}\" cannot be null or empty."; //"NullOrEmptyErrorTemplate";
        private const string NullErrorTemplateResource = "\"{0}\" cannot be null."; //"NullErrorTemplate";


        // Taken from InternalHostUserInterface.
        static internal Type GetFieldType(FieldDescription field)
        {
         // Exception ignoredException;
            Type result = null;

         // if (result == null && !string.IsNullOrEmpty(field.ParameterAssemblyFullName))
         // {
         //     result = LanguagePrimitives.ConvertStringToType(
         //         field.ParameterAssemblyFullName, out ignoredException);
         // }
         // if (result == null && !string.IsNullOrEmpty(field.ParameterTypeFullName))
         // {
         //     result = LanguagePrimitives.ConvertStringToType(
         //         field.ParameterTypeFullName, out ignoredException);
         // }
        ...
        private static class ConsoleHostUserInterfaceStrings
        {
            public static string PromptHelp = "(Type !? for Help.)";
            public static string PromptTypeLoadErrorTemplate = "Cannot display the prompt for \"{0}\" because type \"{1}\" cannot be loaded.";
            public static string RankZeroArrayErrorTemplate = "The field \"{0}\" is a zero rank array.";
            public static string PromptFieldPromptInputSeparatorTemplate = "{0}: ";
            public static string PromptParseOverflowErrorTemplate = "Cannot recognize \"{1}\" as a {0} due to an overflow error.";
            public static string PromptParseFormatErrorTemplate = "Cannot recognize \"{1}\" as a {0} due to a format error.";
            public static string PromptNoHelpAvailableErrorTemplate = "No help is available for {0}.";
            public static string PromptUnrecognizedCommandErrorTemplate = "\"{0}\" cannot be recognized as a valid Prompt command.";
            public static string DefaultChoiceForMultipleChoices = "(default is \"{0}\")";
            public static string DefaultChoicesForMultipleChoices = "(default is \"{0}\")";
            public static string DefaultChoicePrompt = "(default is \"{0}\"): ";
            public static string PromptCanceledError = "The prompt was canceled.";
            public static string ChoiceMessage = "Choice[{0}]: ";
            public static string PromptForChoiceHelp = "[?] Help ";

        } // end class ConsoleHostUserInterfaceStrings
    }
}   // namespace 
