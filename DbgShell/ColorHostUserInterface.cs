...
namespace MS.DbgShell
{
    ...
    /// ColorHostUserInterface is copied from that.
    /// 
    /// </summary>
    ...
    internal partial class ColorHostUserInterface : System.Management.Automation.Host.PSHostUserInterface
    {
        ...
        internal ColorHostUserInterface(ColorConsoleHost parent)
        {
            ...
            warningFormatString = "WARNING: {0}"; // ConsoleHostUserInterfaceStrings.WarningFormatString;
        ...
        public override SecureString ReadLineAsSecureString()
        {
            ...
            Util.Assert(secureResult != null, "ReadLineSafe did not return a SecureString");
        ...
        {
            Util.Assert(!string.IsNullOrEmpty(printToken),
        ...
        internal List<string> WrapText(string text, int maxWidthInBufferCells)
        {
            ...
            do
            {
                ...
                if (!valid)
                {
                    ...
                    {
                        l = line.ToString();
                        Util.Assert(RawUI.LengthInBufferCells(l) <= maxWidthInBufferCells, "line is too long");
                        result.Add(l);
                    }
                    break;
                }
                ...
                else
                {
                    ...
                    Util.Assert(w < e.Current.CellCount, "width remaining should be less than size of word");

                    line.Append(e.Current.Text.Substring(0, w));

                    l = line.ToString();
                    Util.Assert(RawUI.LengthInBufferCells(l) == maxWidthInBufferCells, "line should exactly fit");
                    result.Add(l);
        ...
        {
            Util.Assert(endIndex >= startIndex, "startIndex must be before endIndex");
            Util.Assert(endIndex >= 0, "endIndex must be positive");
            Util.Assert(startIndex >= 0, "startIndex must be positive");
            Util.Assert(startIndex < text.Length, "startIndex must be within the string");
            Util.Assert(endIndex <= text.Length, "endIndex must be within the string");
            ...
            while (startIndex < endIndex)
            {
                ...
                Util.Assert(RawUI.LengthInBufferCells(w.Text) <= maxWidthInBufferCells, "word should not exceed max");
                result.Add(w);
        ...
        public override void WriteWarningLine(string message)
        {
            WriteWrappedLine(
                ...
                Util.Sprintf(warningFormatString, message));
        }
        ...
        public override void WriteProgress(Int64 sourceId, ProgressRecord record)
        {
            if (record == null)
            {
                Util.Assert(false, "WriteProgress called with null ProgressRecord");
            }
            else
            {
                bool matchPattern;
                ...
                if (matchPattern)
                {
                    // No access to internal ProgressRecord copy constructor. [danthom]
                    //record = new ProgressRecord(record) {CurrentOperation = currentOperation};
                    record = CopyProgressRecord( record, currentOperation );
                }
        ...
        /// 
        /// </returns>
        internal string ReadLineWithTabCompletion(bool useUserDefinedCustomReadLine)
        {
            ConsoleHandle handle = ConsoleControl.GetActiveScreenBufferHandle();
        ...
        internal void HandleThrowOnReadAndPrompt()
        {
            ...
            {
                throw new InvalidOperationException( "DbgShell is in NonInteractive mode. Read and Prompt functionality is not available." );
            }
        }
