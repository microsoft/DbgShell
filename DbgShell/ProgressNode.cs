...
namespace MS.DbgShell
{
    ...
    ProgressNode : ProgressRecord
    {
        ...
        {
            Util.Assert(record.RecordType == ProgressRecordType.Processing, "should only create node for Processing records");
        ...
        {
            Util.Assert(strCollection != null, "strCollection should not be null");
            Util.Assert(indentation >= 0, "indentation is negative");
            Util.Assert(this.RecordType != ProgressRecordType.Completed, "should never render completed records");
            ...
            switch (Style)
            {
                ...
                case RenderStyle.Invisible:
                    ...
                    break;
                default:
                    Util.Assert(false, "unrecognized RenderStyle value");
                    break;
            }
        }
        ...
        RenderFull(ArrayList strCollection, int indentation, int maxWidth, PSHostRawUserInterface rawUI, bool isFullPlus)
        {
            ...
            strCollection.Add(
                Utils.TruncateToBufferCellWidth(
                    rawUI, Util.Sprintf(" {0}{1} ", indent, this.Activity), maxWidth));
        ...
        {
            string oldDescription = Util.Sprintf(" {0}{1} ", indent, description);
            string newDescription;

            do
            {
                newDescription = Utils.TruncateToBufferCellWidth(rawUi, oldDescription, maxWidth);
                ...
                {
                    oldDescription = Util.Sprintf(" {0}{1}", indent, oldDescription.Substring(newDescription.Length));
                }
            } while (isFullPlus);
        }
        ...
        RenderCompact(ArrayList strCollection, int indentation, int maxWidth, PSHostRawUserInterface rawUI)
        {
            ...
            strCollection.Add(
                Utils.TruncateToBufferCellWidth(
                    rawUI,
                    Util.Sprintf(" {0}{1} ", indent, this.Activity), maxWidth));
            ...
            if (PercentComplete >= 0)
            {
                percent = Util.Sprintf("{0}% ", PercentComplete);
            }
            ...
            if (!String.IsNullOrEmpty(CurrentOperation))
            {
                strCollection.Add(
                    Utils.TruncateToBufferCellWidth(
                        rawUI,
                        Util.Sprintf(" {0}{1} ", indent, this.CurrentOperation), maxWidth));
            }
        }
        ...
        internal int LinesRequiredMethod(PSHostRawUserInterface rawUi, int maxWidth)
        {
            ...
            switch (Style)
            {
                ...
                default:
                    Util.Assert(false, "Unknown RenderStyle value");
                    break;
            }
